...
def conv_batchnorm_relu_block(input_tensor, nb_filter, kernel_size=3, name_suffix=None):
    """
    ...
    if name_suffix is None:
        ...
        x = Conv2D(nb_filter, (kernel_size, kernel_size),
                   padding='same', name="conv2d_" + name_suffix)(input_tensor)
        x = BatchNormalization(axis=2, name="batchnorm_" + name_suffix)(x)
        x = Activation('relu', name="activ_relu_" + name_suffix)(x)

    return x



def subpixel_conv2d(input_shape, scale=4, name=None):
    """
